...
<section class="content-card" data-rup-form-page data-mode="<?= esc($mode) ?>" data-record-id="<?= esc($recordId ?? '') ?>">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mb-4">
        <div>
            ...
            <p class="text-secondary mb-0">Preencha os dados da pessoa vinculada à RUP.</p>
        </div>
        <a class="btn btn-outline-secondary" href="<?= site_url('rup') ?>">Voltar</a>
    </div>
    ...
    <div class="alert d-none mt-4 mb-0" id="rup-form-message" role="status" aria-live="polite"></div>
</section>

<script>
window.rupApi = {
    list: '<?= site_url('api/rup') ?>',
    showBase: '<?= site_url('api/rup') ?>',
    create: '<?= site_url('api/rup') ?>',
    updateBase: '<?= site_url('api/rup') ?>'
};
</script>
